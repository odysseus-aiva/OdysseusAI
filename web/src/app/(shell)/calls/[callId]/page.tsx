...
function CostBreakdown({ cost }: { cost: CallCost }) {
  if (cost.pricingModel === 'omni') {
    return <OmniCostBreakdown cost={cost} />;
  }
...
function OmniCostBreakdown({ cost }: { cost: CallCost }) {
    ...
    >
      ...
      <div className="flex flex-col gap-2 pt-0.5">
        <div className="flex items-center gap-3">
          <span className="flex w-16 shrink-0 items-center gap-1.5 text-[11.5px]" style={{ color: 'var(--color-text-muted)' }}>
            <span aria-hidden className="rounded-full" style={{ width: 6, height: 6, background: 'var(--color-accent)' }} />
            Omni
          </span>
        ...
        <p className="text-[10.5px] pt-1 leading-[1.4]" style={{ color: 'var(--color-text-faint)', borderTop: '1px solid var(--color-border)', paddingTop: 8 }}>
          Single all-in rate covering hearing, reasoning, retrieval, and speech. No separate STT/LLM/TTS billing.
        </p>
      </div>
    </div>
  );
